...
(define (install-raise-package) ;install raise functions 
  ...
  (define (raise-rational x)
    (make-real (/ ((get 'rational 'numer) x) ((get 'rational 'denom) x))))
  (define (raise-real x)
...
(define (raise x y)
  (let ((tag-x (type-tag x))
       (tag-y (type-tag y)))
    (cond ((eq? tag-x tag-y) x)
          ((< (level tag-x) (level tag-y)) (raise ((get 'raise tag-x) x) y)))))
      ...
      (if proc
          ...
          (if (= 2 (length args))
              (let ((t1 (car type-tags))
                    ...
                    (a1 (car args))
                    (a2 (cadr args)))
                (cond ((< (level t1) (level t2)) (apply-generic op (raise a1 a2) a2))
                      ((> (level t1) (level t2)) (apply-generic op a1 (raise a2 a1)))))
