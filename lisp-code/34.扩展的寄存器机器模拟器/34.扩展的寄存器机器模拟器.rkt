...
(define (find-label label label-list)
  (cond ((null? label-list) #f)
        ((eq? (car label-list) label) #t)
        (else (find-label label (cdr label-list)))))
(define (label-list text cur-list)
  (cond
    ((null? text) cur-list)
    ((symbol? (car text))
     (if (find-label (car text) cur-list)
         (cons #t cur-list)
         (label-list (cdr text) (cons (car text) cur-list))))
    (else (label-list (cdr text) cur-list))))

(define (make-machine register-names ops controller-text)
  ;参数分别为寄存器列表、操作列表和指令序列
  (let ((labels (label-list controller-text '())))
    (if (and (not (null? labels)) (eq? (car labels) #t))
        (begin (displayln "label error in machine") 'error)
...
(define (make-swap inst machine pc)
  (let ((reg1 (get-register machine (cadr inst)))
    ...
    (lambda ()
      (let ((reg-value1 (get-contents reg1))
            (reg-value2 (get-contents reg2)))
        (set-contents! reg1 reg-value2)
        (set-contents! reg2 reg-value1)
        (advance-pc pc)))))
