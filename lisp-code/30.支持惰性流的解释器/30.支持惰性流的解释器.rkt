...
(define (my-eval exp env)
  (cond ;((equal? exp '(cond ((and (> 5 3) (> 6 2) (* 3 4))))) 12)
         ...
         ((quoted? exp) (text-of-quotation exp env));单引号表达式
...
(define (text-of-quotation-lazy exp env) 
   (define (quotation->cons exp) 
     ...
     (let ((text (cadr exp))) 
       (if (pair? text) 
         (my-eval (quotation->cons text) env) 
         text)))
...
(my-eval '(define (cons x y) 
          (lambda (m) (m x y))) glb-env) 
  
 (my-eval '(define (car z) 
          (z (lambda (p q) p))) glb-env) 
  
 (my-eval '(define (cdr z) 
          (z (lambda (p q) q))) glb-env) 
  
  
 (my-eval '(define (list-ref items n) 
          (if (= n 0) 
              (car items) 
              (list-ref (cdr items) (- n 1)))) glb-env) 
  
 (my-eval '(define (map proc items) 
          (if (null? items) 
              '() 
              (cons (proc (car items)) 
                    (map proc (cdr items))))) glb-env) 
  
 (my-eval '(define (scale-list items factor) 
          (map (lambda (x) (* x factor)) 
               items)) glb-env) 
  
 (my-eval '(define (add-lists list1 list2) 
          (cond ((null? list1) list2) 
                ((null? list2) list1) 
                (else (cons (+ (car list1) (car list2)) 
                            (add-lists (cdr list1) (cdr list2)))))) glb-env)
