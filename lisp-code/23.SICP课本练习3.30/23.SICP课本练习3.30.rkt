...
(define (ripple-carry-adder wire-lstA wire-lstB wire-lstS wireCN)
  (define (carry-adder lstA lstB lstS wireC cin)
    (if (null? (cdr lstA))
        (full-adder (car lstA) (car lstB) cin (car lstS) wireC)
        (let ((cout (make-wire)))
          (full-adder (car lstA) (car lstB) cin (car lstS) cout)
          (carry-adder (cdr lstA) (cdr lstB) (cdr lstS) wireC cout))))
  (let ((cin (make-wire)))
    (carry-adder wire-lstA wire-lstB wire-lstS wireCN cin)
    (void)))
